...
int main()
{
    ...
    /*cout<<~x<<endl;;
    y=1;//=00001
    y=1<<4;//=10000=16 bit shifted in left for 4 bits=1*2^4
    y=y<<4;//y*2^4;
    z=16;//=10000
    z=z>>4;//=00001 bit shifted for 4 bit in right side=z/2^4
    cout<<y<<endl;
    cout<<z<<endl;

    a >>= 1; //is similar to:
    a = (a >> 1); //">>" is the bitwise right-shift operator.
                  //Same goes for "<<", which is the bitwise left-shift operator.
